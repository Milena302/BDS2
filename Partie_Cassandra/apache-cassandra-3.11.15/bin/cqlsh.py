...
parser.add_option('--no_compact', action='store_true',
                  help='No Compact', default=False)
parser.add_option("-u", "--username", help="Authenticate as user.")
parser.add_option("-p", "--password", help="Authenticate using password.")
parser.add_option('-k', '--keyspace',
                  help='Authenticate to the given keyspace.')
...
parser.add_option(
    "--cqlshrc", help="Specify an alternative cqlshrc file location.")
...
warnings.filterwarnings(
    'always', category=cql3handling.UnexpectedTableStructure)
...
def insert_driver_hooks():
    ...
    cassandra.cqltypes.DateType.deserialize = staticmethod(
        deserialize_date_fallback_int)
...
class Shell(cmd.Cmd):
        ...
        if use_conn:
            ...
            self.conn = Cluster(contact_points=(self.hostname,), port=self.port, cql_version=cqlver,
                                ...
                                ssl_options=sslhandling.ssl_settings(
                                    hostname, CONFIG_FILE) if ssl else None,
    ...
    def get_table_meta(self, ksname, tablename):
        ...
        if tablename not in ksmeta.tables:
            if ksname == 'system_auth' and tablename in ['roles', 'role_permissions']:
                ...
                raise ColumnFamilyNotFound(
                    "Column family %r not found" % tablename)
        else:
    ...
    def get_view_meta(self, ksname, viewname):
        ...
        if viewname not in ksmeta.views:
            raise MaterializedViewNotFound(
                "Materialized view %r not found" % viewname)
            ...
            try:
                ...
                if readline.__doc__ is not None and 'libedit' in readline.__doc__:
                    ...
                    readline.parse_and_bind(
                        "bind '" + self.completekey + "' rl_complete")
    ...
    def onecmd(self, statementtext):
        ...
        try:
            statements, endtoken_escaped = cqlruleset.cql_split_statements(
                statementtext)
    ...
    def handle_parse_error(self, cmdword, tokens, parsed, srcstr):
        if cmdword.lower() in ('select', 'insert', 'update', 'delete', 'truncate',
            ...
            return self.perform_statement(cqlruleset.cql_extract_orig(tokens, srcstr))
        if parsed:
            self.printerr('Improper %s command (problem at %r).' %
                          (cmdword, parsed.remainder[0]))
        else:
            self.printerr('Improper %s command.' % cmdword)

    def do_use(self, parsed):
        ksname = parsed.get_binding('ksname')
        success, _ = self.perform_simple_statement(
            SimpleStatement(parsed.extract_orig()))
        if success:
    ...
    def perform_statement(self, statement):
        stmt = SimpleStatement(statement, consistency_level=self.consistency_level,
                               serial_consistency_level=self.serial_consistency_level, fetch_size=self.page_size if self.use_paging else None)
        ...
        if future:
            ...
            if self.tracing_enabled:
                try:
                    ...
                    self.printerr(
                        "Unable to fetch query trace: %s" % (str(err),))
        ...
        try:
            ...
            try:
                ...
                raise ObjectNotFound(
                    "%r not found in keyspace %r" % (name, ks))
    ...
    def perform_simple_statement(self, statement):
        ...
        future = self.session.execute_async(
            statement, trace=self.tracing_enabled)
        result = None
        try:
            result = future.result()
        except CQL_ERRORS, err:
            self.printerr(unicode(err.__class__.__name__) +
                          u": " + err.message.decode(encoding='utf-8'))
        except Exception:
            import traceback
        ...
        if statement.query_string[:6].lower() == 'select':
            self.print_result(
                result, self.parse_for_select_meta(statement.query_string))
        elif statement.query_string.lower().startswith("list users") or statement.query_string.lower().startswith("list roles"):
            self.print_result(
                result, self.get_table_meta('system_auth', 'roles'))
        elif statement.query_string.lower().startswith("list"):
            self.print_result(result, self.get_table_meta(
                'system_auth', 'role_permissions'))
        elif result:
            # CAS INSERT/UPDATE
            self.writeresult("")
            self.print_static_result(result, self.parse_for_update_meta(
                statement.query_string), with_header=True, tty=self.tty)
        self.flush_output()
        return True, future
            ...
            while True:
                ...
                if result.current_rows or isFirst:
                    ...
                    self.print_static_result(
                        result, table_meta, with_header, tty)
    ...
    def print_static_result(self, result, table_meta, with_header, tty):
        ...
        formatted_names = [self.myformat_colname(
            name, table_meta) for name in column_names]
        if not result.current_rows:
            # print header only
            self.print_formatted_result(
                formatted_names, None, with_header=True, tty=tty)
            return
        ...
        if result.column_types:
            ...
            cql_types = [CqlType(cql_typename(t), ks_meta)
                         for t in result.column_types]

        formatted_values = [map(self.myformat_value, [
                                row[column] for column in column_names], cql_types) for row in result.current_rows]
        ...
        if self.expand_enabled:
            ...
            self.print_formatted_result(
                formatted_names, formatted_values, with_header, tty)
    ...
    def print_formatted_result(self, formatted_names, formatted_values, with_header, tty):
        ...
        # print header
        if with_header:
            header = ' | '.join(hdr.ljust(w, color=self.color)
                                for (hdr, w) in zip(formatted_names, widths))
        ...
        for row in formatted_values:
            line = ' | '.join(col.rjust(w, color=self.color)
    ...
    def print_formatted_result_vertically(self, formatted_names, formatted_values):
        ...
        max_val_width = max(
            [n.displaywidth for row in formatted_values for n in row])

        # for each row returned, list all the column-value pairs
        for row_id, row in enumerate(formatted_values):
            self.writeresult("@ Row %d" % (row_id + 1))
            self.writeresult(
                '-%s-' % '-+-'.join(['-' * max_col_width, '-' * max_val_width]))
            for field_id, field in enumerate(row):
                column = formatted_names[field_id].ljust(
                    max_col_width, color=self.color)
    ...
    def describe_keyspace(self, ksname):
        print
        self.print_recreate_keyspace(
            self.get_keyspace_meta(ksname), sys.stdout)
        print
    ...
    def describe_columnfamilies(self, ksname):
        ...
        if ksname is None:
            for k in self.get_keyspaces():
                ...
                print '---------%s' % ('-' * len(name))
                cmd.Cmd.columnize(self, protect_names(
                    self.get_columnfamily_names(k.name)))
                print
        else:
            cmd.Cmd.columnize(self, protect_names(
                self.get_columnfamily_names(ksname)))
            print
    ...
    def describe_function(self, ksname, functionname):
        ...
        print
        ksmeta = self.get_keyspace_meta(ksname)
        functions = filter(lambda f: f.name == functionname,
                           ksmeta.functions.values())
        if len(functions) == 0:
            raise FunctionNotFound(
                "User defined function %r not found" % functionname)
    ...
    def describe_aggregate(self, ksname, aggregatename):
        ...
        print
        ksmeta = self.get_keyspace_meta(ksname)
        aggregates = filter(lambda f: f.name == aggregatename,
                            ksmeta.aggregates.values())
        if len(aggregates) == 0:
            raise FunctionNotFound(
                "User defined aggregate %r not found" % aggregatename)
  ...
  """
        ...
        if what == 'functions':
            ...
            ksname = self.cql_unprotect_name(
                parsed.get_binding('ksname', None))
            functionname = self.cql_unprotect_name(
                parsed.get_binding('udfname'))
            ...
            ksname = self.cql_unprotect_name(
                parsed.get_binding('ksname', None))
            aggregatename = self.cql_unprotect_name(
                parsed.get_binding('udaname'))
            ...
            if not name:
                name = self.cql_unprotect_name(
                    parsed.get_binding('idxname', None))
            if not name:
                name = self.cql_unprotect_name(
                    parsed.get_binding('mvname', None))
    ...
    def do_copy(self, parsed):
        ...
        if direction == 'FROM':
            task = ImportTask(self, ks, table, columns, fname,
                              opts, self.conn.protocol_version, CONFIG_FILE)
        elif direction == 'TO':
            task = ExportTask(self, ks, table, columns, fname,
                              opts, self.conn.protocol_version, CONFIG_FILE)
        else:
    ...
    def do_tracing(self, parsed):
        ...
        """
        self.tracing_enabled = SwitchCommand("TRACING", "Tracing").execute(
            self.tracing_enabled, parsed, self.printerr)
    ...
    def get_help_topics(self):
        topics = [t[3:] for t in dir(self) if t.startswith(
            'do_') and getattr(self, t, None).__doc__]
        ...
        for t in topics:
            if t.lower() in self.get_help_topics():
                ...
                if urlpart is not None:
                    ...
                    if len(webbrowser._tryorder) == 0:
                        self.printerr(
                            "*** No browser to display CQL help. URL for help topic %s : %s" % (t, url))
...
class SwitchCommandWithValue(SwitchCommand):
    ...
    def execute(self, state, parsed, printerr):
        binary_switch_value = SwitchCommand.execute(
            self, state, parsed, printerr)
        switch = parsed.get_binding('switch')
        try:
...
def read_options(cmdlineargs, environment):
    ...
    optvalues = optparse.Values()
    optvalues.username = option_with_default(
        configs.get, 'authentication', 'username')
    optvalues.password = option_with_default(
        rawconfigs.get, 'authentication', 'password')
    optvalues.keyspace = option_with_default(
        configs.get, 'authentication', 'keyspace')
    ...
    optvalues.field_size_limit = option_with_default(
    ...
    optvalues.timezone = option_with_default(
        configs.get, 'ui', 'timezone', None)

    optvalues.debug = False
    optvalues.file = None
    optvalues.ssl = option_with_default(
        configs.getboolean, 'connection', 'ssl', DEFAULT_SSL)
    optvalues.no_compact = False
    optvalues.encoding = option_with_default(
        configs.get, 'ui', 'encoding', UTF8)

    optvalues.tty = option_with_default(
        configs.getboolean, 'ui', 'tty', sys.stdin.isatty())
    optvalues.protocol_version = option_with_default(
        configs.getint, 'protocol', 'version', None)
    optvalues.cqlversion = option_with_default(
        configs.get, 'cql', 'version', None)
    optvalues.connect_timeout = option_with_default(
        configs.getint, 'connection', 'timeout', DEFAULT_CONNECT_TIMEOUT_SECONDS)
    optvalues.request_timeout = option_with_default(
        configs.getint, 'connection', 'request_timeout', DEFAULT_REQUEST_TIMEOUT_SECONDS)
    optvalues.execute = None

    (options, arguments) = parser.parse_args(cmdlineargs, values=optvalues)

    hostname = option_with_default(
        configs.get, 'connection', 'hostname', DEFAULT_HOST)
...
def setup_cqlruleset(cqlmodule):
    ...
    cqlruleset.commands_end_with_newline.update(
        cqlshhandling.my_commands_ending_with_newline)
